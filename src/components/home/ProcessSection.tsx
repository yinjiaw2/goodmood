...
export default function ProcessSection() {
  ...
  return (
    <section id="process" className="w-full bg-white py-24 px-8 scroll-mt-16">
      <div className="max-w-7xl mx-auto">
        {/* Top: two-column header (reference style) */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-end mb-20">
          ...
          {/* Right: subtitle */}
          <p
            className="text-lg text-gray-500 leading-relaxed max-w-md lg:pb-2"
            style={fontStyle}
          >
            {t("process.subtitle")}
          </p>
        ...
        <div className="relative">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {steps.map((step, i) => (
              <div
                key={i}
                className="flex flex-col items-start lg:items-center gap-5"
              >
                {/* Icon circle */}
                <div
                  className="relative flex items-center justify-center w-14 h-14 rounded-full shrink-0 text-white z-10"
